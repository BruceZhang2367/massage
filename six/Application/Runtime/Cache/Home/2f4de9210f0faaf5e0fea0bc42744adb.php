...
<div class="i_bg">
    ...
    <div class="content">
        ...
        <div class="pro_des">
        	<div class="des_name">
            	<p><?php echo ($goods_data["goods_name"]); ?></p>
                “开业巨惠，北京专柜直供”，不光低价，“真”才靠谱！
            </div>
            <div class="des_price">
            	本店价格：<b>￥<?php echo ($goods_data["shop_price"]); ?></b><br />
                消费积分：<span>28R</span>
            </div>
            <?php if(is_array($new_attr)): foreach($new_attr as $key=>$v): ?><div class="des_choice">
            	<span class="fl"><?php echo ($v["name"]); ?>：</span>
                <ul>
                <?php if(is_array($v["value"])): foreach($v["value"] as $key=>$vv): ?><li val="<?php echo ($key); ?>"><?php echo ($vv); ?><div class="ch_img"></div></li><?php endforeach; endif; ?>
                </ul>
            ...
            <div class="des_join">
                ...
                <span class="fl"><a class="addCart"><img src="images/j_car.png" /></a></span>
            </div>            
        </div>    
            ...
            <div class="des_border">
                ...
                <div class="des_con" id="p_attribute">
                	...
                	<table border="0" align="center" style="width:100%; font-family:'宋体'; margin:10px auto;" cellspacing="0" cellpadding="0">
                      <tr>
                        <?php if(is_array($value)): foreach($value as $key=>$v): ?><td><?php echo ($v["attr_name"]); ?>:<?php echo ($v["attr_values"]); ?></td><?php endforeach; endif; ?>
                      </tr>
                      <!-- <tr>
                        <td>商品毛重：160.00g</td>
                        <td>商品产地：法国</td>
...
<script type="text/javascript">
    jq(document).on("click",".des_choice ul li",function(){
        jq(this).siblings().removeClass("checked");
        jq(this).addClass("checked");
    })
    jq(document).on("click",".addCart",function(){
        var count="<?php echo count($new_attr); ?>";
        var goods_id="<?php echo $_GET['goods_id'] ?>";
        var num=jq(".n_ipt").val();
        //属性值id
        var goods_attr_id=new Array();
        //属性和属性值
        var goods_values=new Array();
        jq(".checked").each(function(){
            goods_attr_id.push(jq(this).attr("val"));
            goods_values.push(jq(this).parent().prev().html()+jq(this).text());
        })
        //判断用户是否选择了所有规格
        if(goods_attr_id.length < count){
            alert("请选择所有规格后再加入购物车");
            return false;
        }
        jq.ajax({
            url:"/six/index.php/Home/Cart/cartAdd",
            data:{goods_id:goods_id,num:num,goods_attr_id:goods_attr_id,goods_values:goods_values},
